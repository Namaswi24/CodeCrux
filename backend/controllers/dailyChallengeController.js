...
const getDailyChallenge = asyncHandler(async (req, res) => {
  const user = await User.findById(req.user.id).populate('solvedProblems'); // Populate to check efficiently
  if (!user) {
    res.status(404);
  ...
  outerLoop: for (const platform of platformsToTry) {
    for (const diff of preferredDifficulty) { // Iterate through difficulties
      ...
      const problemsFromDB = await fetchProblemsFromPlatform(platform, { // Now queries your DB
        difficulty: diff,
        tags: randomTag,
        limit: 20, // Fetch a decent number of candidates
      });
      ...
      if (problemsFromDB && problemsFromDB.length > 0) {
        ...
        if (unsolvedProblems.length > 0) {
          ...
          break outerLoop; // Found a problem
        }
      }
    }
  }
